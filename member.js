...
document.querySelector('#member-register-form')?.addEventListener('submit', async event => { event.preventDefault(); const values = Object.fromEntries(new FormData(event.currentTarget)); values.termsAccepted = values.termsAccepted === 'on'; values.privacyAccepted = values.privacyAccepted === 'on'; try { const data = await customerRequest('/api/v1/customer/register', { method: 'POST', body: JSON.stringify(values) }); verificationEmail = values.email.trim().toLowerCase(); showMemberView('verify'); say(memberMessage, demoCode(data, '電子郵件')); } catch (error) { say(memberMessage, error.message); } });
