...
async function refreshOverview() {
  const [products, orders, consignments] = await Promise.all([api.listProducts(), api.listOrders(), api.listConsignments()]);
  updateOverview(products, orders, consignments);
}
render();
...
async function saveBanner(event) {
  ...
  const form = new FormData(formElement); const placement = { '首頁主視覺': 'hero', '首頁橫幅 970 × 250': 'home_leaderboard', '商品側欄 300 × 600': 'product_sidebar', '手機橫幅': 'mobile_banner' }[form.get('placement')];
  const submitForReview = document.activeElement.id === 'publish-new';
  controls.forEach(control => { control.disabled = true; });
  try { const image = form.get('image'); if (!(image instanceof File) || !image.size) throw new Error('請選擇 Banner 圖片。'); const intent = await api.createBannerUploadIntent({ fileName: image.name, contentType: image.type, contentLength: image.size }); const upload = await fetch(intent.uploadUrl, { method: 'PUT', headers: { 'content-type': image.type }, body: image }); if (!upload.ok) throw new Error('Banner 圖片上傳失敗，請重新選擇檔案。'); const asset = await api.completeBannerUpload(intent.id); const created = await api.createBanner({ name: form.get('name'), kind: form.get('type'), placement, priority: Number(form.get('priority')), startsAt: new Date(form.get('startsAt')).toISOString(), endsAt: new Date(form.get('endsAt')).toISOString(), targetUrl: form.get('targetUrl') || null, imageKey: asset.imageKey }); if (submitForReview) await api.submitBanner(created.id); banners = (await api.listBanners()).map(apiBanner); render(document.querySelector('[data-filter].selected').dataset.filter); await refreshOverview(); dialog.close(); formElement.reset(); showToast(submitForReview ? 'Banner 已送交審核。' : 'Banner 草稿已儲存。'); } catch (error) { showToast(error.message || 'Banner 儲存失敗。'); } finally { controls.forEach(control => { control.disabled = false; }); }
}
table.addEventListener('click', async event => { const id = event.target.dataset.approve || event.target.dataset.submit || event.target.dataset.disable; if (!id) return; try { if (event.target.dataset.approve) await api.approveBanner(id); if (event.target.dataset.submit) await api.submitBanner(id); if (event.target.dataset.disable) await api.disableBanner(id); banners = (await api.listBanners()).map(apiBanner); render(document.querySelector('[data-filter].selected').dataset.filter); await refreshOverview(); showToast('Banner 狀態已更新，總覽待辦已同步。'); } catch (error) { showToast(error.message || '狀態更新失敗。'); } });
if (api.enabled) {
