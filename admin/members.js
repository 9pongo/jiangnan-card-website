...
function renderAudit(data) { auditRows.innerHTML = data.map(log => `<tr><td>${new Date(log.createdAt).toLocaleString('zh-TW')}</td><td><b>${esc(log.actor)}</b></td><td>${esc(log.action)}</td><td>${esc(log.entityType)}</td></tr>`).join(''); }
