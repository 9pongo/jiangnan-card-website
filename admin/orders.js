...
const names = { pending_payment: '待付款', paid: '已付款', cancelled: '已取消', fulfilled: '已履約' };
function render(data) { rows.innerHTML = data.map(order => `<tr><td><b>${order.orderNumber}</b></td><td>${order.customerEmail}</td><td>${order.itemCount}</td><td><b>${money(order.amountDueCents)}</b></td><td><span class="status ${order.status === 'pending_payment' ? 'review' : order.status === 'paid' ? 'live' : ''}">${names[order.status] || order.status}</span></td></tr>`).join(''); }
render(demoOrders);
